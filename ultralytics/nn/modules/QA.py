...
class QuadrangleAttention(nn.Module):
    ...
    def forward(self, x):
        shortcut = x
        device = x.device  # 加在 forward 一开始
        b, C, h, w = x.shape
        ...
        image_reference = torch.stack(torch.meshgrid(image_reference_w, image_reference_h,indexing='ij'), 0).permute(0, 2,
                                                                                                       1).unsqueeze(
            0)  # 2, h, w
        ...
        if h > ws or w > ws:
            ...
            sampling_projc = torch.cat([
                sampling_projc.reshape(-1, window_num_h, window_num_w, 1, 2).to(x.device),
                torch.ones_like(zero_vector).cuda().reshape(-1, window_num_h, window_num_w, 1, 1).to(x.device)
            ], dim=-1)



            shear_matrix = torch.stack([
                torch.ones_like(zero_vector).to(device),
                sampling_shear[..., 0],
                sampling_shear[..., 1],
                torch.ones_like(zero_vector).to(device)
            ...
            scales_matrix = torch.stack([
                sampling_scales[..., 0],
                torch.zeros_like(zero_vector).to(device),
                torch.zeros_like(zero_vector).to(device),
                sampling_scales[..., 1],
            ], dim=-1).reshape(-1, window_num_h, window_num_w, 2, 2)
            ...
            rotation_matrix = rotation_matrix.to(dtype)
            shear_matrix = shear_matrix.to(dtype)
            scales_matrix = scales_matrix.to(dtype)
            ...
            window_coords_pers = torch.cat([
                window_coords.flatten(-2, -1).to(device), torch.ones(1, window_num_h, window_num_w, 1, ws * ws).to(device)
            ], dim=-2)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = QuadrangleAttention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, window_size=window_size,
            rpe=rpe, coords_lambda=coords_lambda
        )

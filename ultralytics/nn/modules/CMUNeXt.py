...
class CMUNeXtDown(nn.Module):
    """
    小目标友好的多分支下采样模块：
    - 分支1：CMUNeXtBlock（DWConv + MLP）+ stride 卷积下采样
    - 分支2：AvgPool 下采样 + 1x1 Conv 对齐通道
    - 融合：concat 后 1x1 Conv 融合，再可选 SE 做通道重标定
    """

    def __init__(self, ch_in, ch_out, k=3, s=2, depth=1, use_se=False):
        """
        Args:
            ch_in (int): 输入通道数（由上一层自动推断）
            ch_out (int): 输出通道数（YAML 中传入）
            k (int): 卷积核大小（用于 local 分支的 CMUNeXtBlock 和 Conv）
            s (int): 下采样步长，一般为 2
            depth (int): CMUNeXtBlock 深度（堆叠次数），默认 1
            use_se (bool): 是否启用 SE 通道注意力
        """
        super().__init__()
        assert s in [2, 4], "CMUNeXtDown 主要用于 stride=2 或 4 的下采样"

        # 分支1：保留你现有的 CMUNeXtBlock 作为局部分支
        self.local = CMUNeXtBlock(ch_in, ch_out, k=k, s=s, depth=depth)

        # 分支2：AvgPool 下采样 + 1x1 Conv 对齐通道
        self.pool = nn.Sequential(
            nn.AvgPool2d(kernel_size=s, stride=s),
            Conv(ch_in, ch_out, k=1, s=1)
        )

        # 融合：concat 后用 1x1 Conv 压回 ch_out 通道
        self.fuse = Conv(2 * ch_out, ch_out, k=1, s=1)

        # 可选 SE 通道注意力
        self.use_se = use_se
        if use_se:
            mid = max(ch_out // 4, 8)  # 防止通道过小时为 0
            self.se = nn.Sequential(
                nn.AdaptiveAvgPool2d(1),
                nn.Conv2d(ch_out, mid, kernel_size=1),
                nn.GELU(),
    ...
    def forward(self, x):
        # 分支1：CMUNeXt 下采样特征
        x_local = self.local(x)  # [B, ch_out, H/s, W/s]
        # 分支2：池化下采样特征
        x_pool = self.pool(x)    # [B, ch_out, H/s, W/s]

        # 融合
        x_cat = torch.cat([x_local, x_pool], dim=1)  # [B, 2*ch_out, H/s, W/s]
        out = self.fuse(x_cat)                       # [B, ch_out, H/s, W/s]
...
if __name__ == '__main__':
    ...
    m = CMUNeXtDown(128, 256, k=3, s=2, depth=1, use_se=False)
    y = m(x)
    print(x.shape, '->', y.shape)  # 期望: torch.Size([1, 128, 160, 160]) -> torch.Size([1, 256, 80, 80])

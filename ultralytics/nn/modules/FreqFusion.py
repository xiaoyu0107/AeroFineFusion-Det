...
try:
    ...
    def carafe(x, normed_mask, kernel_size, group=1, up=1):
            ...
            normed_mask = normed_mask.reshape(b, kernel_size * kernel_size, m_h, m_w).unsqueeze(1)
...
#             self.compressed_channels,
#             lowpass_kernel ** 2 * self.up_group * self.scale_factor ** 2,
#             self.encoder_kernel,
#             padding=int((self.encoder_kernel - 1) * self.encoder_dilation / 2),
#             dilation=self.encoder_dilation,
#             groups=1)
...
#                 self.compressed_channels,
#                 highpass_kernel ** 2 * self.up_group * self.scale_factor ** 2,
#                 self.encoder_kernel,
#                 padding=int((self.encoder_kernel - 1) * self.encoder_dilation / 2),
#                 dilation=self.encoder_dilation,
#                 groups=1)
...
#         else:
#             self.register_buffer('hamming_lowpass', torch.ones(1, 1, lowpass_kernel, lowpass_kernel))
#             self.register_buffer('hamming_highpass', torch.ones(1, 1, highpass_kernel, highpass_kernel))
#
#         self.init_weights()
#
#     def init_weights(self):
...
#         n, mask_c, h, w = mask.size()
#         mask_channel = int(mask_c / (kernel ** 2))
#         mask = mask.view(n, mask_channel, -1, h, w)
#         mask = F.softmax(mask, dim=2, dtype=mask.dtype)
#         mask = mask.view(n, mask_channel, kernel, kernel, h, w)
#         mask = mask.permute(0, 1, 4, 5, 2, 3).reshape(n, -1, kernel, kernel)
...
#
#         if self.use_high_pass:
#             mask_hr_feat = self.content_encoder2(compressed_hr_feat)
#             mask_hr = self.kernel_normalizer(mask_hr_feat, self.highpass_kernel, self.scale_factor, self.hamming_highpass)
#             hr_feat = hr_feat - carafe(hr_feat, mask_hr, self.highpass_kernel, self.up_group, 1)
#             if self.hr_residual:
#                 hr_feat = hr_feat + hr_feat
#
#         mask_lr_feat = self.content_encoder(compressed_hr_feat)
#         mask_lr = self.kernel_normalizer(mask_lr_feat, self.lowpass_kernel, self.scale_factor, self.hamming_lowpass)
#         lr_feat = carafe(lr_feat, mask_lr, self.lowpass_kernel, self.up_group, 2)
#
#         if self.feature_resample:
...
class FreqFusionWithBiFPN(nn.Module):
    def __init__(self,
                 hr_channels,
                 lr_channels,
                 scale_factor=2,
    ...
    def forward(self, hr_feat, lr_feat):
        _, hr_out, lr_out = self.freqfusion(hr_feat, lr_feat)
        w = torch.relu(self.weight)
        w = w / (w.sum() + self.eps)
        # BiFPN-style weighted fusion
        fused = w[0] * hr_out + w[1] * lr_out
        return fused
...
if __name__ == '__main__':
    ...
    model = FreqFusionWithBiFPN(hr_channels=128, lr_channels=128)

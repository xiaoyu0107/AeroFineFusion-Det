...
class QA(nn.Module):
    def __init__(self, input_resolution, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., window_size=7,
                 rpe='v2', coords_lambda=20):
        super().__init__()
        ...
        self.window_coords = torch.stack(torch.meshgrid(expanded_base_coords_w, expanded_base_coords_h), 0).permute(0,
                                                                                                                    2,
                                                                                                                    1).reshape(
            1, 2, window_num_h, ws, window_num_w, ws).permute(0, 2, 4, 1, 3, 5).cuda()
